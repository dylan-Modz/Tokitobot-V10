...
async function starttokito(tokito, upsert) {
  try {
    ...
    for (const info of upsert?.messages || []) {
      ...
      if (!from)
        continue
      // V9: status só é lido quando visualizarmsg estiver ativo; fora disso é ignorado.
      if (isStatus) {
...
${colors.cyan('╰──. ݁ ⛧ ₊ ⊹ 🧊 . ݁ ˖ ❆ິ̸ . ݁──╯')}`)
      ...
      // Todos os comandos agora são carregados pelos plugins.
      // Se nenhum plugin reconhecer o comando, usa a busca por similaridade.
      try {
