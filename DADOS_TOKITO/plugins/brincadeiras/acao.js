...
return reply(cfg.erro || mess.padraoUso({
emoji: '👤',
titulo: cfg.nome || 'BRINCADEIRA',
uso: `${prefix}${cfg.comandos[0]} @usuario`,
descricao: 'Marque uma pessoa para continuar.'
}))
if (cfg.naoMesmo && alvo === sender)
return reply(mess.padraoAviso({
titulo: 'OUTRA PESSOA',
descricao: 'Escolha outra pessoa para esta brincadeira.'
}))
await reagir(from, cfg.emoji || '🎭').catch(() => {
})
const numero = String(alvo).split('@')[0]
const autor = String(sender).split('@')[0]
const textoAcao = typeof cfg.caption === 'function' ? cfg.caption({
numero,
autor,
ctx
...
emoji: cfg.emoji || '🎭',
titulo: cfg.nome || cfg.comandos[0] || 'BRINCADEIRA',
linhas: [
{ rotulo: '🎭 𝙰𝙲̧𝙰̃𝙾', valor: textoAcao }
]
})
