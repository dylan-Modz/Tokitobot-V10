 ...
 * ============================================================
 *  Arquivo: div.js
 *  Função : Controle da divulgação e payment normal
 *  Dev    : Dylan Modz
 * ============================================================
 */
 ...
 * ============================================================
 *
 * Usa requestPaymentMessage normal do WAProto.
 *
 * O valor não faz parte do comando e não é exibido pelo sistema.
 * amount1000 fica no valor padrão 0 do protobuf.
 *
 * O modo como o WhatsApp renderiza o card depende da versão
 * do cliente e da disponibilidade do recurso de pagamentos.
 * ============================================================
 */

function montarPayment(texto) {
...
async function enviarPayment(tokito, jid, texto, quoted = null) {
  const mensagemTexto = String(texto || '').trim()

  if (!mensagemTexto) {
    throw new Error('A mensagem do payment está vazia.')
  }
  ...
  if (
    ...
    throw new Error('relayMessage não está disponível nesta conexão.')
  }

  const mensagem = generateWAMessageFromContent(
    jid,
    montarPayment(mensagemTexto),
    {
...
async function carregarGrupos(tokito) {
  if (
    !tokito ||
    typeof tokito.groupFetchAllParticipating !== 'function'
  ) {
    throw new Error(
      'groupFetchAllParticipating não está disponível nesta build da Baileys.'
    )
  }
  ...
  const grupos = Object.values(bruto || {})
    .filter(
      g =>
        g?.id &&
        String(g.id).endsWith('@g.us')
    )
    .map(g => ({
      id: g.id,
      nome: nomeGrupo(g),
      participantes: Array.isArray(g.participants)
...
function catalogoAtual() {
  return Array.isArray(runtime.catalogo)
    ? runtime.catalogo
    : []
}
...
function definirIntervalo(segundos) {
  ...
  if (
    ...
    throw new Error(
      `O intervalo deve ficar entre ${MIN_INTERVALO} e ${MAX_INTERVALO} segundos.`
    )
  }
...
function limpar() {
  if (runtime.executando) {
    throw new Error(
      'Pare a campanha antes de limpar.'
    )
  }
...
function obterStatus() {
  return {
    ...
    limites: {
      maxPorRodada: MAX_POR_RODADA,
      minIntervalo: MIN_INTERVALO,
      maxIntervalo: MAX_INTERVALO
    }
  }
}
...
async function preview(tokito, jid) {
  ...
  if (!state.texto) {
    throw new Error(
      'Nenhuma mensagem de divulgação foi configurada.'
    )
  }

  return enviarPayment(
    tokito,
    jid,
    state.texto
  )
...
async function enviar(tokito, quantidade) {
  ...
  if (!state.texto) {
    throw new Error(
      'Configure a mensagem com divmsg antes de enviar.'
    )
  }
  ...
  if (
    ...
    limite < 1 ||
    limite > MAX_POR_RODADA
  ) {
    throw new Error(
      `A quantidade deve ser de 1 até ${MAX_POR_RODADA}.`
    )
  }
